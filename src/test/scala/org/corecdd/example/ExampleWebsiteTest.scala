...
class ExampleWebsiteTestWithSlashTest extends AbstractExampleWebsiteTest {
  def prefix = "/"
}
@RunWith(classOf[JUnitRunner])
class ExampleWebsiteTestWithPrefixTest extends AbstractExampleWebsiteTest {
  def prefix = "/prefix/"
}

abstract class AbstractExampleWebsiteTest extends AbstractWebsiteTest with BeforeAndAfterAll {
  import Reportable._
  def prefix: String
  ...
  val cddHandler = new CddHandler(prefix, project)
  ...
  def projectPageAtIndex = {
    go to (host + prefix);
    new ProjectPage(List(project, report))
  }
